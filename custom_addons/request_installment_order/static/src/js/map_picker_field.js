...
export class MapPickerWidget extends Component {
  static template = "request_installment_order.MapPickerWidget";
  static props = {
    ...standardFieldProps,
  };
  ...
  initializeMap() {
    ...
    if (this.state.latitude && this.state.longitude) {
      this.marker = window.L.marker([this.state.latitude, this.state.longitude])
        .addTo(this.map)
        .bindPopup(
          `Lat: ${this.state.latitude.toFixed(
            6
          )}<br>Lng: ${this.state.longitude.toFixed(6)}`
        )
        .openPopup();
    }
  ...
  addGeolocationControl() {
    ...
    const GeolocationControl = window.L.Control.extend({
      onAdd: () => {
        ...
        container.innerHTML =
          '<i class="fa fa-location-arrow" style="margin: 5px;"></i>';
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          this.marker
            .bindPopup(
              `Current Location<br>Lat: ${lat.toFixed(6)}<br>Lng: ${lng.toFixed(
                6
              )}`
            )
            .openPopup();
        ...
        (error) => {
          ...
          alert(
            "Unable to get current location. Please check your browser settings."
          );
        }
      );
    }
...
// Register the widget
registry.category("fields").add("map_picker_v3", MapPickerWidget);