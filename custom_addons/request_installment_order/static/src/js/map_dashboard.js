...
export class CustomerMapDashboard extends Component {
    ...
    async loadCustomerData() {
        ...
        try {
            ...
            const records = await this.orm.searchRead(
                ...
                ["id", "title", "description", "latitude", "longitude", "location_display", "active"]
            );
            ...
            records.forEach((record, index) => {
                try {
                    ...
                    console.error(`Error adding marker for customer ${record.title}:`, error);
                }
            });
    ...
    async processFallbackRecords(records) {
        ...
        records.forEach((record) => {
            try {
                ...
                console.error(`Error adding fallback marker for customer ${record.title}:`, error);
            }
        });
    ...
    addCustomerMarker(record) {
        ...
        if (isNaN(lat) || isNaN(lng)) {
            console.warn(`Invalid coordinates for customer ${record.title}: lat=${record.latitude}, lng=${record.longitude}`);
            return null;
        }

        if (lat < -90 || lat > 90 || lng < -180 || lng > 180) {
            console.warn(`Out of range coordinates for customer ${record.title}: lat=${lat}, lng=${lng}`);
            return null;
        }
        ...
        try {
            ...
            const popupContent = `
                <div class="customer-popup" style="min-width: 250px;">
                    <h4 style="margin: 0 0 15px 0; color: #1f77b4; border-bottom: 2px solid #1f77b4; padding-bottom: 8px;">
                        <i class="fa fa-user" style="margin-right: 8px;"></i>${record.title}
                    </h4>
                    ${record.description ? `
            ...
            marker.on('click', () => {
                console.log(`Clicked customer: ${record.title} (ID: ${record.id})`);
            });

            return marker;

        } catch (error) {
            console.error(`Error creating marker for ${record.title}:`, error);
            return null;
        }
    }
...
CustomerMapDashboard.template = "request_installment_order.CustomerMapDashboard";

// Register the action
registry.category("actions").add("request_installment_order.map_dashboard", CustomerMapDashboard);